...
func plunder():
	for item in _booty:
		...
		item.apply_impulse(
			(
				Vector2.UP * Global.ppt * _rng.randf_range(5, 10)
				+ Vector2.RIGHT * Global.ppt * _rng.randf_range(-1, 1)
			)
		)
...
func throw_padlock():
	...
	padlock.apply_impulse(
		Vector2.UP * Global.ppt + Vector2.RIGHT * Global.ppt * _rng.randf_range(-1, 1)
	)
