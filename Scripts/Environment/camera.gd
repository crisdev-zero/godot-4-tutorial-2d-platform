...
func _on_subject_changed_direction(is_facing_left: bool):
	...
	_look_ahead_tween = create_tween().set_trans(_look_ahead_trans_type).set_ease(
		_look_ahead_ease_type
	)
	_look_ahead_tween.tween_property(
		self,
		"_look_ahead_distance",
		_offset.x * (-1 if is_facing_left else 1),
		_look_ahead_duration
	)
...
func _on_subject_landed(floor_height):
	...
	_floor_height_tween = create_tween().set_trans(_floor_height_trans_type).set_ease(
		_floor_height_ease_type
	)
